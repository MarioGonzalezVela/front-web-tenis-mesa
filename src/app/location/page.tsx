...
export default function LocationPage() {
  ...
  const markAsVisited = async (locationId: number) => {
    if (!token) {
    ...
    try {
      const res = await fetch("http://localhost:8000/api/visited-locations", {
        method: "POST",
        headers: {
  ...
  const submitReview = async (locationId: number) => {
    ...
    try {
      ...
      setReviews(prev => ({ ...prev, [locationId]: '' })); // Vacía el campo de reseña después de guardarla
    } catch (error) {
        ...
        {locations
          ...
          .map(location => (
            <div key={location.id} className="border border-gray-700 rounded-2xl p-4 shadow-lg hover:shadow-xl bg-gray-900 transition duration-200">
              <h2 className="text-xl font-semibold text-gray-200 mb-4">{location.name}</h2>
              <p className="text-gray-400 mb-2">📍 {location.address}</p>
              <p className="text-gray-400 mb-2">📞 {location.phone}</p>
